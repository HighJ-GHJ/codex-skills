...
This is a Codex-first repo-scoped skill. The skill directory also includes `agents/openai.yaml` for UI-facing metadata, but the workflow remains source-repo oriented rather than plugin-oriented.
...
```bash
python -m prepare_web_gpt_handoff.prepare --mode strategy_research --topic "..." --goal "..." --focus-point "..." --must-include README.md --max-bundle-tokens 4096
```
...
```bash
python -m prepare_web_gpt_handoff.confirm --handoff handoffs/<handoff_id>
```
...
- Prefer token budgets over character budgets; install `tiktoken` when exact counting matters
- If the runtime must guarantee exact token counting, enable `--require-exact-tokens` or `PREPARE_WEB_GPT_HANDOFF_REQUIRE_EXACT_TOKENS=1`
- Keep visible outputs in `handoffs/`; do not hide the real handoff package in an internal-only directory
- Do not automate browser actions
- Do not assume Codex can remember the handoff later; always rely on `handoff_id`

Compatibility note:

- `scripts/*.py` still exist, but they are now compatibility wrappers; the real implementation and recommended entrypoint are the package modules under `prepare_web_gpt_handoff`
